...
class MyActions(models.Model):
    ...
    def _compute_is_printed (self):
        for rec in self:
            if self.env['sinergis.myactions.printed'].search_count([('user_id', '=', self.env.sudo().user.id),('model_type', '=', rec.origin),('model_id', '=', rec.link_id)]) > 0:
                rec.is_printed = True
            else:
