...
class TrainingOtherDocument(models.Model):
    ...
    def on_change_type(self):
        if self.env['training.other_document'].search_count([('type', '=', 'CGV')]) >= 1 and self.type == "CGV":
                raise ValidationError("Le document 'CGV' existe déjà")
        if self.env['training.other_document'].search_count([('type', '=', 'Livret de formation')]) >= 1 and self.type == "Livret de formation":
                raise ValidationError("Le document 'Livret de formation' existe déjà")
        if self.env['training.other_document'].search_count([('type', '=', 'Diagnostic initial')]) >= 1 and self.type == "Diagnostic initial":
