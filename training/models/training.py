...
class Training(models.Model):
    ...
    def _compute_duration_hours (self):
        for rec in self:
            ...
            if rec.sale_order_line_id.product_uom.name == "Heures":
                rec.duration_hours = rec.sale_order_line_id.product_uom_qty
            else:
    ...
    def send_invitation_individual(self):
        if Training.verifiation_fields(self.training_id):
            ...
            if self.email:
                ...
                attach_obj = self.env['ir.attachment']
                attachment_ids = []
                result_binary = self.env['training.other_document'].sudo().search([('type', '=', 'Livret de formation')], limit=1).file
                if result_binary:
                    attach_data = {
